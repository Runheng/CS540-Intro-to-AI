...
class SimpleConvNet(nn.Module):
    ...
    def __init__(self, input_shape=(32, 32), num_classes=100):
        ...
        # 2 convs
        layers.append(nn.Conv2d(3, 32, kernel_size=5, stride=1))
        layers.append(nn.ReLU(inplace=True))
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        layers.append(nn.Conv2d(32, 128, kernel_size=5, stride=1))
        layers.append(nn.ReLU(inplace=True))
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        
        layers.append(nn.Flatten())
        # 3 FCs
        layers.append(nn.Linear(3200, 1024))
        layers.append(nn.ReLU(inplace=True))
        ...
        layers.append(nn.Flatten())
        layers.append(nn.Linear(32768, 512))
        layers.append(nn.ReLU(inplace=True))
        layers.append(nn.Linear(512, 128))
        layers.append(nn.ReLU(inplace=True))
        layers.append(nn.Linear(128, num_classes))
